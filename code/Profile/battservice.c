...
const uint8_t BATT_ADC_CH = 1;

#define BATT_ADC_EN_PIN GPIO_Pin_23
#define BATT_ADC_ON()     ((R32_PB_OUT) |= BATT_ADC_EN_PIN)
#define BATT_ADC_OFF()     ((R32_PB_OUT) &= (~BATT_ADC_EN_PIN))

// ADC voltage levels scalar by 0.5
#define BATT_ADC_LEVEL_4_2V            2100
#define BATT_ADC_LEVEL_3_3V            1650
 ...
 * LOCAL VARIABLES
 */
signed short BATT_RoughCalib_Value = 0; // ADC粗调偏差值
...
static uint16_t battMinLevel = BATT_ADC_LEVEL_3_3V; // For VDD measurements
static uint16_t battMaxLevel = BATT_ADC_LEVEL_4_2V; // For VDD measurements
 ...
 *
 * @param   adc_ch - ADC Channel, e.g. HAL_ADC_CHN_AIN6  // uint8_t adc_ch,
...
void Batt_Setup()
{
    GPIOB_ModeCfg(BATT_ADC_EN_PIN, GPIO_ModeOut_PP_5mA); // PortB - EN
    BATT_ADC_ON();

    GPIOA_ModeCfg(BATT_ADC_PIN, GPIO_ModeIN_Floating);
    ADC_ExtSingleChSampInit(SampleFreq_3_2, ADC_PGA_1_2);
    ADC_ChannelCfg(BATT_ADC_CH);

    BATT_RoughCalib_Value = ADC_DataCalib_Rough();
...
static uint8_t battMeasure(void)
{
    ...
    adc /= 3;
    adc += BATT_RoughCalib_Value;
    adc = (adc/1024 - 1) * 1050; // 转为mV
    // #ifdef DEBUG
    //     PRINT("BATT adc: %d\r\n", adc);
    // #endif
